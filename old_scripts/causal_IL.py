...
from stable_baselines.common.tf_layers import conv_to_fc
...
def kf(obs,kill_feats=[],original_len=None,on=False):
...
def get_deepq_expert(env,path=None,overwrite=False,verbose=True,resume_training=False,
    ...
    if type(env) is str:
        ...
        env_name = env.unwrapped.envs[0].unwrapped.spec.id if hasattr(env.unwrapped,'envs') else env.unwrapped.spec.id
    if env_name in ['BeamRiderNoFrameskip-v4','BreakoutNoFrameskip-v4','EnduroNoFrameskip-v4','PongNoFrameskip-v4','QbertNoFrameskip-v4','SeaquestNoFrameskip-v4','SpaceInvadersNoFrameskip-v4','MsPacmanNoFrameskip-v4','BipedalWalker-v2','LunarLander-v2','LunarLanderContinuous-v2']:
        print('Loading {}'.format(env_name))
        algo = {'BeamRiderNoFrameskip-v4':'acktr',
                'BreakoutNoFrameskip-v4':'acktr',
                'EnduroNoFrameskip-v4':'dqn',
                'PongNoFrameskip-v4':'dqn',
                'QbertNoFrameskip-v4':'acer',
                'SeaquestNoFrameskip-v4':'dqn',
                'SpaceInvadersNoFrameskip-v4':'dqn',
                'MsPacmanNoFrameskip-v4':'acer',
                'BipedalWalker-v2':'sac',
                'LunarLander-v2':'dqn',
                'LunarLanderContinuous-v2':'sac'}[env_name]
        log_path = 'CIL_venv/lib/rl_baselines_zoo/trained_agents/'+algo
        model_path = find_saved_model(algo, log_path, env_name, load_best=False)
        print('Loading {}'.format(env_name))
    ...
    if type(env) is str: #gave env_name rather than env
        if env_mode=='Hist':
            env = HistoryAddWrapper(gym.make(env_name))
        else:
    ...
    pi_E = lambda obs: reshape_action(e_model(obs[np.newaxis,:len(env.reset())]))
    return pi_E
...
def get_policy_trajs(env,act,N_traj=1,path=None,overwrite=False,H=1e8,T=1e8,t0s=None,
                     ...
                     init_acts= None,gif_path=None,preprocess=False):
    '''
    ...
    if path is not None:
        if os.path.exists(path) and overwrite==False:
            ...
            if len(trajs)<N_traj:
                print('Beginning generation of {} more trajs (found {}).'.format(N_traj-len(trajs),len(trajs)))
            else:
    ...
    for n_traj in range(N_traj-len(trajs)):
        traj = []
        ...
        if atari:
            viewer = env.unwrapped.envs[0].viewer
        else:
            viewer = env.viewer
        state = env.state if not atari else None
        episode_rew, t = 0,0
        if t0s is not None:
            t = t0s[n_traj]
            T_max = min(t+H,T)-1
            ...
            T_max = T-1
        while not (done or t>T_max):
            obs = prepro(obs) if preprocess else obs
            action = act(obs) if not atari else [act(obs[0])]
            if render:
                time.sleep(0.02)
                if not atari:
                    ...
                    if gif_path is not None:
                        ...
                        arr = arr.reshape(viewer.height, viewer.width, 4)[::-1, :, 0:3]
                        img_array.append(arr)
                else:
                    env.render()
            new_obs, rew, done, _ = env.step(action)
            new_state = env.state if not atari else None
            traj.append({'o':obs,'a':action,'r':rew,'op':new_obs,'t':t,'s':state,'sp':new_state})
            state,obs = new_state,new_obs
            t += 1
            episode_rew += rew
    ...
    if path is not None:
        ...
        pickle.dump(trajs,open(path2,'wb'))
        if verbose:
...
def nature_cnn_keras(inputs,seed=None):
    ...
    return out
def keras_NN(S_dim,A_dim,H_dims=[8],linear=True,seed=None,discrete_A=True,n_components=1,cnn=False):
    '''NN builder func
    ...
    '''
    print(S_dim,A_dim,H_dims)
    initializer = tf.glorot_uniform_initializer(seed=seed)
    if discrete_A:
        if cnn:
            inp = tf.keras.Input(shape=S_dim)
            feats = nature_cnn_keras(inp,seed=seed)
            logits = tf.keras.layers.Dense(units=A_dim, name='pi')(feats)
...
def train_model(model,inputs,outputs,loss,learning_rate,N_epoch=20,batch_size=32,
    ...
    for epoch in range(N_epoch):
        ...
        while n_steps<steps_per_epoch:
            ...
            if n_steps/print_freq>print_count:
                print_count += 1
                #print(model(inputs)[:2],outputs[:2])
                print(epoch_loss_avg.result().numpy(),loss_value.numpy(),n_steps,epoch)
...
def reshape_agg_data_old(trajs,env,alg='BC',pi_E=None,trajs_E=None,x_old=None,y_old=None,
    ...
    for k in range(K): 
        ...
        for t_L in range(T):
            ...
            x.append(prepro(kf(trajs[k][t_L]['o'],kill_feats,original_dim))) #State
            ...
            if alg in ['ALICE-FAIL','ALICE-Cov-FAIL']:
                ...
                for a in range(A_dim):
                    ...
                    y.append([np.pad(model_L(x[-1][None])[0],(0,A_pad)), #current policy action probabilities
                              ...
                              np.pad(kf(trajs_E[k][t_E]['o'],kill_feats,original_dim),(0,S_pad)), #expert state
    ...
    if x_old is not None:
        if len(x_old)>0:
            if len(x)>0:
                x,y = np.vstack([x_old,np.squeeze(x)]),np.vstack([y_old,np.squeeze(y)])
            else:
                x,y = x_old,y_old
    else:
...
def online_IL(alg,pi_E_path=None,env_name=None,add_history=True,N_expert_traj=5,
    ...
    H_dims = [16]
    batch_size = 1024
    learning_rate = 0.0001 #atari opt = 0.0001, non-atari opt = .1
    ...
    alg_ident = ''.join([alg,'+H' if add_history else '','-S' if DaD_snippets else '',
                         ...
                         '-re{}'.format(ratio_estimator) if alg[:9]=='ALICE-Cov' else '',
                         '-fm{}'.format({'poly 2':'P2','poly 1':'P1','poly 3':'P3','RBF Kernel':'RBF','RFF':'RFF'}[adversary_feature_map]) if alg[-4:]=='FAIL' else ''])
    ...
    if env_name in ATARI_ENVS:
        atari = True
        env = create_test_env(env_name, is_atari=True, log_dir=None,)
    else:
        atari = False
        env = gym.make(env_name)
    ...
    if isinstance(env.action_space,Discrete):
        discrete = True
        reshape_action = lambda model_out: discrete_policy(model_out)
        action_probability = lambda model_out,a_inds : tf.gather_nd(model_out,a_inds)
    else:
        discrete = False
        reshape_action = lambda action_vec: gaussian_policy(model_out)

    pi_E = get_deepq_expert(env,pi_E_path,overwrite=False)
    
    demo_path = 'IL_experts/'+env_name+noise_str+'_demo_trajs.pkl'
    test_path = 'IL_experts/'+env_name+noise_str+'_validation_trajs.pkl'
    get_policy_trajs(env,pi_E,N_traj=max(N_expert_traj),path=demo_path) #Precompute and save
    ...
    ##LOSS FUNCTIONS
    def FAIL_loss(model,inputs,outputs,training,f=None,weights_E=None,weights_samp=None):
        ...
        #action_weight = tf.reduce_sum(a_onehot*model(inputs),axis=1)*tf.cast(A_dim,tf.float32)
    ...
    def regression_loss(model,inputs,outputs,training,f=None,weights_E=None,weights_samp=None):
        weights_samp = tf.ones((len(inputs))) if weights_samp is None else weights_samp
        return tf.compat.v1.losses.mean_squared_error
    def cross_entropy(model,inputs,outputs,training,f=None,weights_E=None,weights_samp=None):
        weights_samp = tf.ones((len(inputs))) if weights_samp is None else weights_samp
        return tf.compat.v1.losses.softmax_cross_entropy(outputs,model(inputs),weights=weights_samp)
    def classification_loss(model,data,labels,f=None,weights_E=None,weights_samp=None):
    ...
    if discrete:
        ...
        default_test_loss = classification_loss
    else:
    ...
    test_loss = {'ALICE-FAIL':FAIL_test_loss,
                 'ALICE-Cov-FAIL':FAIL_test_loss}.get(alg,default_test_loss)
    warm_start = True
    ratio_estimator = {'KDE':density_ratios.KernelDensityEstimator(random_state=opt_seed,warm_start=warm_start),
                       'KLR':density_ratios.KernelLogisticRegression(random_state=opt_seed,warm_start=warm_start),
                       'PLR':density_ratios.PolynomialLogisticRegression(random_state=opt_seed,warm_start=warm_start,degree=int(ratio_estimator[3:]) if len(ratio_estimator)>3 else 2),
                       'VLR':density_ratios.VanillaLogisticRegression(random_state=opt_seed,warm_start=warm_start)}[ratio_estimator[:3]]
    ...
    if alg in ['BC','DaD1','DaD20','DaD21','ALICE-Cov2','ALICE-Cov','ALICE-FAIL','ALICE-Cov-FAIL']:
        for demo_num_ind in range(len(N_expert_traj)):
            ...
            model = keras_NN(Feat_dim,A_dim,H_dims,linear=linear,seed=opt_seed,cnn=atari)
            obs = env.reset()
            print(obs.shape)
            st_t = time.time()
            train_model_p(model,states_E,labels_E,loss=cross_entropy)
            print('Finished in {:.1f} min'.format((time.time()-st_t)/60))
            ...
            for DaD_iter in range(1,N_DaD_iter):
                ...
                if DaD_snippets:
                    ...
                    if set_s0:
                        ...
                        init_states = None
                        pair_traj_E = False
                        trajs_L = get_policy_trajs(env,pi_L,N_traj=N_expert_traj[demo_num_ind],preprocess=True)
                ...
                if alg in ['ALICE-FAIL','ALICE-Cov-FAIL']:
                    for i in range(len(labels_L)):
                        if (states_E[int(labels_L[i,-1,0])]!=labels_L[i,4,:Feat_dim[0]]).any():
                            print('Error @ i:{:d} states_E {:.2f} labels_L {:.2f}'.format(i,states_E[int(labels_L[i,-1,0]),0],labels_L[i,-2,0]))
                            print(states_E[int(labels_L[i,-1,0])],labels_L[i,-2,:])
            ...
            r[demo_num_ind], err[demo_num_ind] = score_policy_p(pi_L,env,states_test,labels_test,verbose=1,policy_name=result_string,loss=classification_loss)
...
def choose_best_hindsight_model(model_list,env,x,y,loss,kill_feats,original_dim,verbose=1,loss_f=None,weights_samp=None,weights_E=None,hindsight_best=True):
    ...
    if len(model_list) == 1 or hindsight_best==False:
        ...
        return len(model_list)-1,score_policy(pi_L,None,x,y,verbose=False,loss=loss,kill_feats=kill_feats,original_dim=original_dim,weights_E=weights_E,weights_samp=weights_samp,loss_f=loss_f)[1]
    ...
    for model_ind in range(len(model_list)):
        ...
        _,hindsight_err[model_ind] = score_policy(pi_L,None,x,y,verbose=False,loss=loss,kill_feats=kill_feats,original_dim=original_dim,weights_E=we,weights_samp=ws,loss_f=loss_f)
        if verbose>1:
...
def score_policy(act,env=None,x=None,y=None,N_traj=10,verbose=True,policy_name='',loss=None,kill_feats=[],original_dim=None,loss_f=None,weights_E=None,weights_samp=None):
    '''scores policy by either computing validation loss or policy reward,
        depending on which of x,y or env is present'''
    if env is not None:
        trajs = get_policy_trajs(env,act,N_traj=N_traj,preprocess=True)
    ...
    if x is not None and y is not None:
        ...
        if loss is None:
            weights_samp = np.ones((len(x))) if weights_samp is None else weights_samp
            cl_err = np.mean([(act(kf(x[i],kill_feats,original_dim))!=np.argmax(y[i]))*weights_samp[i] for i in range(n_samp)]) ###TODO  Score non-categorical as 
        else:
            cl_err = loss(act,np.array([kf(x[i],kill_feats,original_dim) for i in range(n_samp)]),y,f=loss_f,weights_E=weights_E,weights_samp=weights_samp)
...
if __name__=='__main__':
    ...
    env_name = ['MountainCar-v0','Acrobot-v1','PongNoFrameskip-v4'][1]
...
#### Things you might want to do (in the order you might want to do them)
    if 0: #Generate a new expert and try it on the environment
        pi_E = get_deepq_expert(env_name,pi_E_path,overwrite=False,verbose=True)
    ...
    if 0:
        ...
        pi_E = get_deepq_expert(env,pi_E_path,overwrite=False)
        get_policy_trajs(env,pi_E,N_traj=40,verbose=2,render=False,overwrite=False,path='IL_experts/'+env_name+'_demo_trajs.pkl')
        get_policy_trajs(env,pi_E,N_traj=10,verbose=2,render=False,overwrite=False,path='IL_experts/'+env_name+'_validation_trajs.pkl')
    ...
    if 1:
        online_IL('ALICE-FAIL',pi_E_path,env_name,add_history=False,N_expert_traj=[6],N_opt=50000,N_epoch=20,verbose=3,linear=False,run_seed=0,opt_seed=0,set_s0=True,ratio_estimator='VLR',reinit_opt=False,N_DaD_iter=4)
    if 0:
...
#            y += comp_w[0,i]/np.sqrt(2*np.pi*std[0,0,i]**2)*np.exp(-.5*(xaxis-mu[0,0,i])**2/std[0,0,i]**2) 
    ...
    if 0:
        ...
        #online_IL('ALICE-Cov-FAIL',pi_E_path,env_name,add_history=True,N_expert_traj=[10],N_opt=20000,N_epoch=3,verbose=2,linear=True,set_s0=True,run_seed=0,opt_seed=0,DaD_snippets=False,thresh=False,adversary_feature_map='poly 2',ratio_estimator='PLR 1')
    ...
    if 0:
        ...
        results_paths = [env_name+'_ALICE-C2-comp_results.pkl']
        ...
        if plot:
            ...
            inds = [i for i in range(len(results))]

            labels = [results[i][0][-1] for i in inds]#θ2   
